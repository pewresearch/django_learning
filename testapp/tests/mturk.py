...
class MTurkTests(DjangoTestCase):
    ...
    def test_mturk(self):
        ...
        for qual_test in QualificationTest.objects.all():
            print(
                "Navigate to https://workersandbox.mturk.com/qualifications/{} to do the qualification test".format(
        ...
        print(
            "Go to workersandbox.mturk.com, find the hits, and do a few, then press 'c' to continue"
        )
        print(
            "If the hits don't appear, it's because the MTurk interface can be buggy."
        )
        print("Check this out instead: https://github.com/jtjacques/mturk-manage")
        print(
            "Also you sometimes need to do more than one assignment for the sandbox API to return anything"
        )
        import pdb
        ...
        HIT.objects.filter(turk_id__isnull=False).filter(
            assignments__isnull=False
        ).update(turk_id=None)
        time.sleep(5)
        self.assertTrue(
            len(mturk.get_worker_blocks()["Maximum annual compensation"]) > 0
        )
        ...
        self.assertTrue(
            Code.objects.filter(assignment__project__isnull=False).count() == 0
        )
