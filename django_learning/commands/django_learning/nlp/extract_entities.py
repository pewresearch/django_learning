...
class Command(DownloadIterateCommand):
    ...
    def iterate(self, data):
        ...
        if self.parameters["document_type"]:
            docs = Document.objects.filter(
                **{"{0}__isnull".format(self.parameters["document_type"]): False}
            )
            ...
            for id in tqdm(
                docs,
                desc="Extracting document entities ({0})".format(
                    self.parameters["document_type"]
                ),
            ):
                yield [id]
    ...
    def parse_and_save(self, id):
        ...
        if not self.options["search_existing"] or doc.entities.count() == 0:
            ...
            try:
                raw_entities = self.extractor.extract(doc.text)
            ...
            entities = []
            for tag, words in list(raw_entities.items()):
                for word in words:
                    try:
                        entities.append(
                            Entity.objects.create_or_update(
                                {"tag": tag, "name": word}, command_log=self.log
                            )
                        )
                    except Exception as e:
                        print(e)

            doc.entities.set(entities)
            doc.save()
