...
from django_learning.utils.dataset_extractors.document_coder_label_dataset import (
    Extractor as DocumentCoderLabelDatasetExtractor,
)
...
class Extractor(DocumentCoderLabelDatasetExtractor):
    ...
    def _additional_steps(self, dataset, **kwargs):
        ...
        dataset = (
            dataset.groupby(["document_id", "coder_id"]).agg(agg_dict).reset_index()
        )  # this is in case a question allows multiple labels; need to consolidate
            ...
            if len(dataset["coder_id"].unique()) > 1:
                ...
                dataset = pandas.concat(
                    [
                        _standardize(group)
                        for _, group in tqdm(
                            dataset.groupby("coder_id"),
                            desc="Standardizing for coder bias",
                        )
                    ]
                )
