...
class Extractor(BasicExtractor):
    ...
    def fit(self, X, y=None, **fit_params):
        ...
        self.model = get_model("TopicModel", app_name="django_learning").objects.get(name=self.params["model_name"])
        self.topic_names = ["topic_{}".format(c) for c in list(self.model.topics.order_by("num").values_list("name", flat=True)) if c not in [None, '']]
