...
class DataFrame(DataFrameQuery):
    ...
    def _extract_dataframe(self):
        ...
        if "min_date" in self.parameters.keys() and is_not_null(
            self.parameters["min_date"]
        ):
            docs = docs.filter(date__gte=self.parameters["min_date"])
        if "max_date" in self.parameters.keys() and is_not_null(
            self.parameters["max_date"]
        ):
            docs = docs.filter(date__lte=self.parameters["max_date"])

        codes = (
            get_model("ClassifierDocumentCode", app_name="django_learning")
            .objects.filter(document__in=docs)
            .values("document_id", "code__value", "code__variable__name")
        )
        if "classifier_name" in self.parameters.keys() and is_not_null(
            self.parameters["classifier_name"]
        ):
