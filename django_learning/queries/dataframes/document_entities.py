...
class DataFrame(DataFrameQuery):
    ...
    def _extract_dataframe(self):
        ...
        if "min_date" in self.parameters.keys(): docs = docs.filter(date__gte=self.parameters["min_date"])
        if "max_date" in self.parameters.keys(): docs = docs.filter(date__lte=self.parameters["max_date"])
        entities = docs.values("entities__pk").annotate(c=Count("pk")).order_by("-c")[:self.parameters["top_n"]]
        entities = get_model("Entity", app_name="django_learning").objects.filter(pk__in=[e['entities__pk'] for e in entities])

        rows = []
        for d in tqdm(docs, desc="Iterating over documents and extracting entity flags", total=docs.count()):
            row = {"pk": d.pk}
            for e in entities:
